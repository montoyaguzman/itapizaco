...
//______________________________________METODOS DE LA LISTA______________________________________
   ...
   void leeDatos() throws JAXBException, IOException{
     ...
     Escuela escuela = (Escuela) um
     .unmarshal(new FileReader("xalumnos.xml"));
    
     for (int i = 0; i < escuela.getAlumnos().toArray().length; i++){      
      //idv=escuela.getAlumnos().get(i).get_Id();
      nombrev=escuela.getAlumnos().get(i).get_Nombre();
      apellidov=escuela.getAlumnos().get(i).get_Apellido();
      edadv=escuela.getAlumnos().get(i).get_Edad();
      estaturav=escuela.getAlumnos().get(i).get_Estatura();
      pesov=escuela.getAlumnos().get(i).get_Peso();
      generov=escuela.getAlumnos().get(i).get_Genero();
      telefonov=escuela.getAlumnos().get(i).get_Telefono();
      correov=escuela.getAlumnos().get(i).get_Correo();
   ...
   void guardaDatos() throws JAXBException, IOException{
      ...
      escuela2.setClave("ITA2016");
      escuela2.setNombre("Instituto Tecnologico de Apizaco");
      
      for (int l=0; l<listaA.size(); l++){
         alumno1=(Alumnos)listaA.get(l);
         idv=alumno1.get_Id();
         nombrev=alumno1.get_Nombre();
         apellidov=alumno1.get_Apellido();
         edadv=alumno1.get_Edad();
         ...
         escuela2.setAlumnos(alumnos);
         
         JAXBContext context = JAXBContext.newInstance(Escuela.class);
         Marshaller m = context.createMarshaller();
         m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
         m.marshal(escuela2, System.out);
          
         Writer w = null;
         try {
            w = new FileWriter("xalumnos.xml");
            m.marshal(escuela2, w);
           }
           finally{
            try {
             w.close();
